...
arima_css(const py::array_t<double> yv, const py::array_t<uint32_t> armav,
  ...
  for (uint32_t _ = 0; _ < arma[5]; ++_) {
    for (uint32_t l = n - 1; l > 0; --l) {
      w[l] -= w[l - 1];
    }
  }

  const uint32_t ns = arma[4];
  for (uint32_t _ = 0; _ < arma[6]; ++_) {
    for (uint32_t l = n - 1; l >= ns; --l) {
      w[l] -= w[l - ns];
    }
  }

  for (uint32_t l = ncond; l < n; ++l) {
    double tmp = w[l];
    ...
    assert(l >= ncond);
    for (uint32_t j = 0; j < std::min(static_cast<size_t>(l - ncond), q); ++j) {
      if (l - j - 1 < 0) {
        continue;
      }
